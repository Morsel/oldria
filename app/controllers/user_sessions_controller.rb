    ...
    if @user_session.save
      ...
      if user.admin? || user.media? || (user.completed_setup? && !is_profile_not_completed?(user))
        ...
        if @restaurants_has_not_setup_fb_tw.blank?
          ...
          flash[:notice] = "<a href='javascript:void(0)' onclick=\"$('html, body').animate({scrollTop: $('#twitter-fieldset').offset().top -50}, 400);\">Get the most out of your account! Hook up twitter and facebook with your restaurant.today.</a>"
