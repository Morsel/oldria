class MediaNewsletterSubscription < ActiveRecord::Base
  ...
  def send_newsletters_to_media
    for user in User.find(:all,:conditions=>["role = 'media'"])
      ...
      unless MediaNewsletterSubscription.menu_items(arrMedia).blank? && MediaNewsletterSubscription.promotions(arrMedia).blank?
        ...
        user.send_later(:send_newsletter_to_media_subscribers,user) if (!user.media_newsletter_setting.opt_out) && (!["Sun","Sat"].include?(Date.today.wday))
  ...
  def self.menu_items(restaurants)
    ...
    menu_items = []
    menu_items = MenuItem.find(:all,:conditions=>["restaurant_id in (?) and (created_at >= ? OR updated_at >= ?) and created_at < ?",restaurants.compact.map(&:id),1.day.ago.beginning_of_day,1.day.ago.beginning_of_day,0.day.ago.beginning_of_day],:order=>"updated_at desc",:limit=>3)      
  ...
  def self.promotions(restaurants)
   promotions = []
    promotions = Promotion.find(:all,:conditions=>["restaurant_id in (?) and (created_at >= ? OR updated_at >= ?) and created_at < ? ",restaurants.compact.map(&:id),1.day.ago.beginning_of_day,1.day.ago.beginning_of_day,0.day.ago.beginning_of_day],:order=>"updated_at desc",:limit=>3)      
