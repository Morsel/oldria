class UserRestaurantVisitor < ActiveRecord::Base
  ...
  def createUserVisitedEmailSetting(user)
...
def send_notification_to_chef_user
    ...
    User.all.each do |user|
      ...
      if checkEmailFrequency(@uves) && !@uves.do_not_receive_email
        ...
        keywords.keys.each do |key|
          if (key == "ALaMinuteAnswer")||(key == "ALaMinuteQuestion")
            ...
            instance_variable_set("@#{key.to_s.downcase.pluralize}", keywords[key].map(&:keywordable).map(&:name))
          end
        end
