class QuestionsController < ApplicationController
  ...
  def index
    ...
    @previous = @chapter.previous_for_user(current_user)
    @next = @chapter.next_for_user(current_user)
  ...
  def chapters
    ...
    @questions_by_chapter = current_user.profile_questions.
        all(:conditions => { :chapter_id => @topic.chapters.map(&:id) }).
        group_by(&:chapter)
  end
